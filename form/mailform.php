...
if($referer != str_replace( $domain, "", $referer )){
...
function MailMain($formConfig){
	...
	// 問い合わせ者のメール生成処理
	if($formConfig["mail"]["userAddress"] != "" && file_exists($formConfig["userMail"])){
		$mailSet = array();
		...
		// メール送信実施
		$result = runMail($mailSet["to"], $mailSet["subject"], $mailSet["body"], $formConfig["mail"]);
		if(!$result){
			// 結果：次の処理に進まない
			return array(
				"result" => false,
	...
	if(file_exists($formConfig["adminMail"])){
		...
		// メール送信先の設定
		if(strpos($formConfig["mail"]["adminAddress"], ',') !== false){
			$to = explode(',',$formConfig["mail"]["adminAddress"]);
		}else{
		...
		// メール送信実施
		$result = runMail($to, $mailSet["subject"], $mailSet["body"], $formConfig["mail"]);
...
function runMail($to, $subject, $body, $mailConfig){
	// AWS SESを使用する場合
	if (@$mailConfig["sendType"] == "ses") {
		return runMailSES($to, $subject, $body, $mailConfig);
	}
...
// SES（Sendmail）の処理
function runMailSES($to, $subject, $body, $mailConfig){
	// ini定義
	$profile   = 'default'; 																		// iniファイルで設定した[]のテキストを設定
	$localFile = dirname(__FILE__) . '/ses/aws_credential_local.ini';								// 
	$path      = (file_exists($localFile)) ? $localFile : str_replace("_local", "", $localFile);	// ini指定
	...
	$ses_config = [
		...
		'region' => 'ap-northeast-1', // SESを設定しているリージョン
		'version' => '2010-12-01',
		'credentials' => $provider
	];

	$fromName   = $mailConfig["fromName"]; 		// 送信元名称
	$from       = $mailConfig["fromAddress"]; 	// 送信元アドレス
	$to         = $mailConfig["adminAddress"]; 	// 送信先アドレス
	$charset    = 'ISO-2022-JP'; 				// 変換先の文字コード
	$source     = $from; 						// 送信元アドレス（SESとしての送信元）

	try {
		// SES用オブジェクト生成
		$client = new Aws\Ses\SesClient($ses_config);
